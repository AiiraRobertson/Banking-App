  ...
  return (
    <div className="relative" onMouseEnter={handleEnter} onMouseLeave={handleLeave}>
      <button
        ...
        onFocus={handleEnter}
        onClick={() => setOpen(o => !o)}
        className="flex items-center gap-1.5 px-4 py-2 text-sm font-medium text-t-secondary hover:text-indigo-600 transition-colors duration-200">
        Resources
      ...
      <div
        className={`absolute right-0 top-full pt-3 w-[42rem] max-w-[calc(100vw-2rem)] z-50 transition-all duration-200 ${
...
function GlobeSection() {
  return (
    <div className="relative w-full max-w-lg mx-auto aspect-square flex items-center justify-center">
      <div className="absolute inset-0 rounded-full bg-gradient-to-br from-indigo-400/20 to-blue-400/20 blur-3xl animate-pulse" />
      <div className="absolute inset-12 rounded-full bg-gradient-to-tr from-purple-400/15 to-cyan-400/15 blur-2xl animate-float" />
...
export default function LandingPage() {
  return (
    <div className="min-h-screen bg-surface overflow-hidden">
      {/* Navigation */}
      <nav className="fixed top-0 w-full bg-[var(--color-bg-surface)]/80 backdrop-blur-md border-b border-b-secondary z-50 transition-all duration-300">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
            <div className="text-center lg:text-left">
              ...
              <div className="mt-10 flex items-center justify-center lg:justify-start gap-4 animate-fade-in-up delay-300">
                ...
                <Link to="/login" className="px-8 py-3.5 bg-surface text-t-secondary font-semibold rounded-xl border border-b-primary hover:bg-hover hover:border-indigo-200 hover:-translate-y-1 hover:shadow-lg transition-all duration-300 text-lg">
                  Sign In
                </Link>
              </div>
      ...
      {/* Features */}
      <section className="py-20 bg-elevated/50">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-16">
            <h2 className="text-3xl sm:text-4xl font-bold text-t-primary">Everything you need to manage your money</h2>
            <p className="mt-4 text-lg text-t-secondary max-w-2xl mx-auto">Powerful features designed for modern banking, accessible from anywhere in the world.</p>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {features.map((f, i) => (
              <div key={f.title} className="bg-surface rounded-2xl p-6 border border-b-secondary hover:shadow-xl hover:shadow-indigo-100/30 hover:-translate-y-1.5 hover:border-indigo-100 transition-all duration-300 group cursor-default" style={{ animationDelay: `${i * 100}ms` }}>
                ...
                <h3 className="text-lg font-semibold text-t-primary mb-2 group-hover:text-indigo-700 transition-colors duration-200">{f.title}</h3>
                <p className="text-t-secondary text-sm leading-relaxed">{f.desc}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="py-20 relative overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative">
          <div className="text-center mb-16">
            ...
            <p className="mt-4 text-lg text-t-secondary max-w-2xl mx-auto">Transfer funds across North America, Europe, and Africa with competitive rates.</p>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {regions.map((r, ri) => (
              <div key={r.name} className="bg-surface rounded-2xl border border-b-secondary p-6 hover:shadow-xl hover:shadow-indigo-100/30 hover:-translate-y-1 hover:border-indigo-100 transition-all duration-300 group" style={{ animationDelay: `${ri * 150}ms` }}>
                <div className="flex items-center gap-3 mb-4 pb-3 border-b border-b-secondary">
                  ...
                  <div>
                    <h3 className="text-lg font-semibold text-t-primary group-hover:text-indigo-700 transition-colors duration-200">{r.name}</h3>
                    <p className="text-xs text-t-muted">{r.countries.length} countries</p>
                  </div>
                </div>
                <div className="space-y-2">
                  {r.countries.map(c => (
                    <div key={c.name} className="flex items-center gap-3 text-sm py-1 px-2 rounded-lg hover:bg-indigo-50/50 transition-colors duration-200">
                      ...
                      <span className="text-t-secondary">{c.name}</span>
                    </div>
                  ))}
                </div>
              </div>
      ...
      <section className="py-20 relative overflow-hidden">
        ...
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center relative">
          <h2 className="text-3xl sm:text-4xl font-bold text-t-primary">Ready to get started?</h2>
          <p className="mt-4 text-lg text-t-secondary">Open your account in minutes and get a $1,000 welcome bonus. No hidden fees, no hassle.</p>
