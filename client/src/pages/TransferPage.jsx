...
export default function TransferPage() {
  ...
  return (
    <div className="w-full max-w-2xl mx-auto px-3 sm:px-4 space-y-4 lg:space-y-6">
      <div>
        <h1 className="text-xl lg:text-2xl font-bold text-t-primary">Transfer</h1>
        <p className="text-sm lg:text-base text-t-tertiary mt-1">Move money between your accounts, another Kapita user, or to any bank in 27 countries</p>
      </div>

      <Link to="/add-money" className="block bg-green-50 border border-green-200 rounded-lg px-3 sm:px-4 py-3 hover:bg-green-100 transition-colors">
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2 sm:gap-4">
          <div className="min-w-0 flex-1">
            <p className="text-xs sm:text-sm font-semibold text-green-900">Need to add funds?</p>
            <p className="text-[10px] sm:text-xs text-green-800 mt-0.5">Top up your account from a card, bank, or crypto wallet.</p>
          </div>
          <span className="text-green-700 font-medium text-xs sm:text-sm shrink-0">Add Money &rarr;</span>
        </div>
      </Link>

      <div className="bg-surface rounded-lg lg:rounded-xl shadow-sm border border-b-secondary">
        <div className="p-3 sm:p-4 lg:p-6">
          {success ? (
            <div className="text-center space-y-3 sm:space-y-4">
              ...
              <h3 className="text-base lg:text-lg font-semibold text-t-primary">{success.message}</h3>
              <p className="text-xs sm:text-sm text-t-tertiary">Reference: {success.referenceId}</p>
              <p className="text-xs sm:text-sm text-t-tertiary">New balance: {formatCurrency(success.newBalance)}</p>
              {success.extra && (
                <div className="text-[10px] sm:text-xs text-t-tertiary space-y-1 bg-elevated rounded-lg p-2 sm:p-3 max-w-sm mx-auto">
              ...
              <button onClick={reset} className="px-4 py-2 bg-indigo-600 text-white text-sm rounded-lg font-medium hover:bg-indigo-700">
                New Transaction
              </button>
            </div>
          ) : (
            <form onSubmit={handleSubmit} className="space-y-3 sm:space-y-4">
              {error && <div className="p-2 sm:p-3 bg-red-50 border border-red-200 text-red-700 rounded-lg text-xs sm:text-sm">{error}</div>}

              <div>
                <label className="block text-xs sm:text-sm font-medium text-t-secondary mb-1.5 sm:mb-2">From Account</label>
                <select
                  value={form.account_id}
                  onChange={e => setForm({ ...form, account_id: e.target.value })}
                  className="w-full px-3 py-2 border border-b-input rounded-lg text-xs sm:text-sm focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 outline-none"
                  required
                >
                ...
                {selectedAccount && <p className="text-[10px] sm:text-xs text-t-muted mt-1">Available: {formatCurrency(selectedAccount.balance)}</p>}
                {lockedForDebit && (
              ...
              <div>
                  <label className="block text-xs sm:text-sm font-medium text-t-secondary mb-1.5 sm:mb-2">Transfer To</label>
                  <div className="flex flex-wrap gap-1.5 sm:gap-2 mb-3">
                    <button type="button" onClick={() => setForm({ ...form, transferMode: 'own' })}
                      className={`px-2.5 sm:px-3 py-1.5 sm:py-1 text-[10px] sm:text-xs rounded-full transition-colors font-medium ${form.transferMode === 'own' ? 'bg-indigo-600 text-white hover:bg-indigo-700' : 'bg-elevated text-t-secondary hover:bg-hover'}`}>
                      My Account
                    </button>
                    <button type="button" onClick={() => setForm({ ...form, transferMode: 'other' })}
                      className={`px-2.5 sm:px-3 py-1.5 sm:py-1 text-[10px] sm:text-xs rounded-full transition-colors font-medium ${form.transferMode === 'other' ? 'bg-indigo-600 text-white hover:bg-indigo-700' : 'bg-elevated text-t-secondary hover:bg-hover'}`}>
                      Other Account
                    </button>
                    <button type="button" onClick={() => setForm({ ...form, transferMode: 'bank' })}
                      className={`px-2.5 sm:px-3 py-1.5 sm:py-1 text-[10px] sm:text-xs rounded-full transition-colors font-medium ${form.transferMode === 'bank' ? 'bg-indigo-600 text-white hover:bg-indigo-700' : 'bg-elevated text-t-secondary hover:bg-hover'}`}>
                      Other Bank
                    </button>
                  </div>
                  ...
                  {form.transferMode === 'other' && (
                    <div className="space-y-2">
                      ...
                      {lookup.status === 'loading' && (
                        <p className="text-[10px] sm:text-xs text-t-muted flex items-center gap-1.5">
                      ...
                      {lookup.status === 'found' && lookup.data && (
                        <div className="px-3 py-2 bg-green-50 border border-green-200 rounded-lg text-xs sm:text-sm flex items-center gap-2">
                          ...
                          <div className="min-w-0">
                            <p className="font-semibold text-green-800 truncate text-xs sm:text-sm">{lookup.data.account_name}</p>
                            <p className="text-[10px] sm:text-xs text-green-700">{lookup.data.bank_name} · {lookup.data.account_type} (****{lookup.data.account_number.slice(-4)})</p>
                          </div>
                        </div>
                      )}
                      {lookup.status === 'not_found' && (
                        <p className="text-[10px] sm:text-xs text-red-600 flex items-center gap-1.5">
                      ...
                      <div className="flex items-center gap-2 pt-1">
                        ...
                        />
                        <label htmlFor="save-beneficiary" className="text-[10px] sm:text-xs text-t-secondary">
